...
The overlay renders one of four distinct Canvas2D animations depending on the highest-priority attention kind present, so the user can tell at a glance what kind of reminder is awaiting action. Each animation is hand-written with perspective projection and depth sorting, kept dependency-free and fully testable in jsdom — the projection math and per-kind geometry are pure, and the canvas draw is driven by that pure frame computation. Every drawable blits the same pre-rendered radial-gradient glow sprite additively (`lighter`), so overlapping particles gain luminosity instead of opacity, and each frame clears the full device-pixel bitmap so displays with `devicePixelRatio > 1` never accumulate residue between frames. The package deliberately does not depend on the three.js library: a WebGL render path is unreachable under the per-file 100% coverage gate (jsdom has no WebGL context), and the runtime CDN `<script>` load used by the dynamic prototype is a network dependency unsuited to a published plugin.

The four scenes read differently at a glance: `approval` is a swirling lens galaxy — a shell of glowing particles rotating in 3D around a breathing core, circled by a tilted orbit ring carrying one bright comet with a fading trail; `plan-review` is a radar sweep — two concentric dashed rings and a bright radial arm rotating like a scanner with a couple of blips; `question` is an echo — a breathing core with thin ripple rings that pulse outward and fade; `completed` is a calm pulse — a large breathing halo with a slow orbiting set of dots, deliberately quiet to read as finished with no urgency. The accent color follows the highest-priority attention kind present (amber for approval, violet for plan review, cyan for question, green for reply completed), matching the scene each kind plays.
