...
Web GUI 的 Token 消费记录仪表盘。固定在 shell 右下角的浮动按钮打开一个面板,展示某一日历日的 Token 消耗,数据跨会话聚合并按 `(provider, model)` 分组:四个不相交的 Token 桶(未缓存输入、缓存读、缓存写、输出)、请求数、平均消耗速度(加权 tokens/秒)、平均 TTFT(首 token 延迟)、平均缓存命中率。面板带日期选择器(默认今天)与刷新操作。
