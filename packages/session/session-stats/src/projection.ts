/**
 * The `sessionStats` projection unit: a pure fold of step boundaries, attempt
 * streams, tool pairs, and assembled assistant messages into whole-log counts
 * and wall times.
 *
 ...
 * time is `step/start` → `assistant/message`, first token is read from each
 * attempt's assembled stream (falling back to the message stream) and the
 * first stamp survives an in-step `llm/retry`, decode spans
  ...
  apply: (state, event) => {
    ...
    switch (event.type) {
      ...
      case 'assistant/attempt': {
        ...
        const stamped = stepTimingOnFirstTokenTime(open, assistantStreamFirstTokenTime(event.data.stream) ?? null)
      ...
      case 'assistant/message': {
        const open = state.openStep
        // One assembled message per step: a stamped message means this is a
        // defensive duplicate, which folds to the identical reference so the
        // first message's figures are preserved.
        if (open === null || open.turn !== event.data.turn || open.step !== event.data.step) return state
        if (open.messageTime !== null) return state
        // A step with no attempt event still reports its first token through
        // the message's own stream; stamping it here keeps TTFT and decode.
        const withFirst = stepTimingOnFirstTokenTime(open, assistantStreamFirstTokenTime(event.data.stream) ?? null)
        const next = stepTimingOnMessage(withFirst, event.time)
