...
describe('token-usage capture fold', () => {
  it('defers the record to step/end so tool wall time is nonzero and correct', async () => {
    ...
    await gap()
    const toolCall = session.append('tool/call', { turn: 1, step: 1, callId: 'call_1', name: 'read', arguments: '{}' })
    await gap(8)
    const toolResult = session.append('tool/result', {
      turn: 1,
      step: 1,
      message: createToolResultMessage({ callId: 'call_1', content: [{ type: 'text', text: 'ok' }], isError: false }),
    }, { surfaceOp: 'append' })
  ...
  it('writes no record for a no-usage step or a cancelled step', async () => {
    ...
    session.append('tool/call', { turn: 1, step: 3, callId: 'call_x', name: 'read', arguments: '{}' })
    session.append('assistant/message', {
      turn: 1,
      step: 3,
      message: createAssistantMessage({
