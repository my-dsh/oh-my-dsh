  ...
  function captureEvent(session: Session, event: SessionEvent): void {
    switch (event.type) {
      ...
      case 'assistant/attempt': {
        ...
        setOpenCapture(session, {
          ...open,
          fold: stepTimingOnFirstTokenTime(open.fold, assistantStreamFirstTokenTime(event.data.stream) ?? null),
        })
        return
      }
      case 'tool/call': {
      ...
      case 'assistant/message': {
        ...
        // the complete tool wall time. A step with no attempt event still
        // reports its first token through the message's own stream.
        const source = event.data.message.source
        setOpenCapture(session, {
          fold: stepTimingOnMessage(
            stepTimingOnFirstTokenTime(open.fold, assistantStreamFirstTokenTime(event.data.stream) ?? null),
            event.time,
          ),
