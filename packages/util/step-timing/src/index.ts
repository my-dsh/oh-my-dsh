 ...
 * relations — `step/start` opens the boundary, the first-token instant read
 * from an attempt or message stream stamps TTFT, matched `tool/call` →
 * `tool/result` pairs accrue tool wall time, and the first assembled
 * `assistant/message` closes the model-time window.
 *
...
export interface StepTimingFold {
  ...
  /** Epoch milliseconds of the first token in the step's attempt/message streams; null before one lands. */
...
/**
 * Stamp the step's first-token instant, read by the consumer from an
 * `assistant/attempt` or `assistant/message` stream (for example with
 * `assistantStreamFirstTokenTime`). Only the first stamp lands — a retry
 * attempt's later instant never moves the first-token boundary.
 * @param fold - the open step's fold state.
 * @param firstTokenTime - epoch milliseconds of the step's first token, or null when the stream carried none.
 * @returns the updated fold, or the identical reference when nothing stamped.
 */
export function stepTimingOnFirstTokenTime(fold: StepTimingFold, firstTokenTime: number | null): StepTimingFold {
  if (fold.firstTokenTime !== null || firstTokenTime === null) return fold
  return { ...fold, firstTokenTime }
}
