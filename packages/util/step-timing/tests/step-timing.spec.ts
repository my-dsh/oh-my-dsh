...
describe('step-timing fold', () => {
  ...
  it('stamps the first-token instant only once and keeps the boundary across a retry attempt', () => {
    let fold = stepTimingOpen(1, 1, 1_000)
    // A tokenless stream changes nothing, down to the reference.
    expect(stepTimingOnFirstTokenTime(fold, null)).toBe(fold)
    fold = stepTimingOnFirstTokenTime(fold, 1_200)
    expect(fold.firstTokenTime).toBe(1_200)
    // A retry attempt's later instant never moves the first-token boundary.
    expect(stepTimingOnFirstTokenTime(fold, 3_000)).toBe(fold)
  ...
  it('clamps negative clock skew to zero for every duration', () => {
    ...
    fold = stepTimingOnFirstTokenTime(fold, 1_500)
  ...
  it('derives decode time only from the first-token-to-message window', () => {
    ...
    const timed = stepTimingOnMessage(stepTimingOnFirstTokenTime(stepTimingOpen(1, 1, 1_000), 1_400), 4_400)
