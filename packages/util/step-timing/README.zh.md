...
不发布运行时不变量 companion：这个纯折叠不拥有事件流或可变运行时数据；其配对与钳制代数由针对受控事件时间的单元测试保证。
...
| `stepTimingOnFirstTokenTime(fold, firstTokenTime)` | 从流中读到的首 token 瞬时打 TTFT；重试 attempt 的更晚瞬时与无 token 的流返回同一引用。 |
...
- **信任消费方的流读取** —— 流中哪个瞬时算首个 token 由调用方决定（共享的 `assistantStreamFirstTokenTime` 助手），调用方首 token 策略的调整会直接改变这里的 TTFT。
