...
把本地墙钟值转换安全地解析为 IANA 时区内的精确 epoch 瞬时。一个零依赖函数 `resolveZonedWallClock` 独家持有此前被 schedule 规则目标与 token-usage 日窗口重复实现的算法：围绕 UTC 形状猜测采样该时区的 UTC 偏移，把每个偏移投影回候选瞬时，只保留 `Intl` 投影能逐字段精确复现所请求值的候选。
