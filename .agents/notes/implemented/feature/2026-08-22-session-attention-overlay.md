...
A user with a backgrounded tab sees the reminder in the tab title and, on returning, sees the persistent 3D panel and each waiting session by name and color until they act on every one. The panel ships as the standalone bundle `@deepseek-ai/dsh-session-attention` (one `cordis.patch.yml` row), installable over any web-surface profile; the assembled-boot snapshot pins its registration and the fixture pending-question row.

## Testing

`attention.client.spec.ts` covers the pure selection (driven by both `SessionListState` and a `useSessionPendingInteraction` snapshot), key, completion logic, and the `computeScene` per-kind geometry (each kind yields a distinct, valid scene); `scene.client.spec.ts` covers the projection math, the sprite-blit `paintFrame`, and the `createAttentionScene` lifecycle (one-frame loop, reduced-motion static frame, no-raf disposal, null-context no-op); `panel.client.spec.tsx` renders `AttentionPanel` against fake `useSessions` and `useSessionPendingInteraction` hooks with an injectable scene factory and covers empty render, every attention kind, the completed theme, the highest-priority kind driving the scene factory, click-to-open, title tagging and restore, the +N tail, reduced motion, the default copy path, and a throwing scene factory. `apply.client.spec.tsx` covers the `shell.overlay` registration, deferred injection, the open-session action, and teardown. `apps/web/tests/session-attention.snapshot.ts` boots the assembled built graph against the keyless fixture transport and pins the overlay's wrap presence, head count, the fixture pending-question row, and the tagged tab title.
